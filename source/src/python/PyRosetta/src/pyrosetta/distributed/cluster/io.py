    ...
    def _write_init_file(self, filename: str) -> None:
        ...
        try:
            dump_init_file(
                ...
                metadata="Generated by PyRosettaCluster",
                overwrite=True,
            )
        except BaseException as ex:
            logging.warning(
                f"`pyrosetta.dump_init_file` did not run successfully, "
                + "so PyRosetta initialization input data was not saved! "
                + "It is recommended to run `pyrosetta.dump_init_file` "
                + "to reproduce PyRosetta initialization options later."
            )
